...
function genRuns() {
    ...
    for (var i = cnt - 1; i >= max; i--) {
        ...
        var s = '<li data-theme="' + (r.isDnf ? 'a' : r.getRerun ? 'e' : '') + '"><a id="run-' + r.runNumber + '-' + r.driverId + '">'
            ...
            + '<p class="ui-li-desc">' +  r.car.description + '<br>PAX: <strong>' + r.timepaxed + '</strong><br></p><span class="ui-li-count">' + ( r.isDnf ? ''  : (r.cones > 1 ? r.cones + ' cones' : (  r.cones == 1 ? r.cones + ' cone' : 'Clean' )  ) ) + ' </span>';
        if (r.isDnf) {
