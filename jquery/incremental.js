...
function genTtod() {
    ...
    for (var i = 0; i < ttod.length; i++) {
        var c = ttod[i];
        html += '<li data-role="list-divider"><h1>' + c.category + '</h1></li>';
        ...
        $.ajax({
            ...
            , success: function (d) {
                ...
                for (rx in d) {
                    ...
                    html.push('<li' + theme + '><a href="#">' + r.rawtime.toFixed(3) + ' (' + r.timepaxed.toFixed(3)  + ')' + r.cones + '</a></li>');
...
function genDriver(driver) {
    ...
    $('#driverinfo').html('<H1><strong>' + driver.name + '</strong></h1><h1> ' + driver.axclass + ' ' + driver.car.number + '</h1><br><h1>' + driver.car.description + '</h1>'  );
...
function genRuns() {
    ...
    for (var i = cnt - 1; i >= max; i--) {
        ...
        var s = '<li data-theme="' + (r.isDnf ? 'a' : r.getRerun ? 'e' : '') + '"><a id="run-' + r.runNumber + '-' + r.driverId + '">'
            ...
            + '<p class="ui-li-desc">' +  r.car.description + '<br><strong>' + r.axclass + ' ' + r.car.number  + '</strong></p>' + ( ( r.getRerun || r.isDnf ||  r.cones == 0  ) ? ' ' : '<span class="ui-li-count">' + (r.cones == 1 ? r.cones + ' cone' : r.cones + ' cones' ) + ' </span>' ) ;
